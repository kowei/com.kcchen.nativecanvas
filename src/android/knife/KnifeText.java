...
import com.kcchen.nativecanvas.colorpicker.builder.ColorPickerClickListener;
import com.kcchen.nativecanvas.colorpicker.builder.ColorPickerDialogBuilder;
...
import com.kcchen.nativecanvas.knife.utils.KnifeData;
...
public class KnifeText extends AppCompatEditText implements TextWatcher {
    protected static final String TAG = KnifeText.class.getSimpleName();
    ...
    public static final int FORMAT_TEXTUP = 0x08;
    public static final int FORMAT_TEXTDOWN = 0x09;
    public static final int FORMAT_FORGROUND = 0x0A;
    public static final int FORMAT_BACKGROUND = 0x0B;
    public static final int FORMAT_STYLE = 0x0C;
    public static final int FORMAT_TEXTSIZE = 0x0D;

    private static final String KNIFE_SPANNABLE = "KnifeSpannable";
    ...
    private KnifeToolPanel toolPanel;
    private Context context;

    private OnClickListener toolPanelClickListener = new OnClickListener() {
        @Override
        public void onClick(View view) {
            ...
            if (viewId == R.id.makeBold) {
                bold(!contains(KnifeText.FORMAT_BOLD));
            } else if (viewId == R.id.makeItalic) {
                italic(!contains(KnifeText.FORMAT_ITALIC));
            } else if (viewId == R.id.makeUnderline) {
                underline(!contains(KnifeText.FORMAT_UNDERLINED));
            } else if (viewId == R.id.makeStrikethrough) {
                strikethrough(!contains(KnifeText.FORMAT_STRIKETHROUGH));
            } else if (viewId == R.id.makeTextUp) {
                textsizeup(!contains(KnifeText.FORMAT_TEXTUP));
            } else if (viewId == R.id.makeTextDown) {
                textsizedown(!contains(KnifeText.FORMAT_TEXTDOWN));
            } else if (viewId == R.id.makeForeground || viewId == R.id.makeBackground) {
                changeColor(view);
            }
        }
    };

    private void changeColor(final View view) {
        ...
        ColorPickerDialogBuilder
                ...
                .setPositiveButton("ok", new ColorPickerClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int selectedColor, Integer[] allColors) {
                        if (view.getId() == R.id.makeForeground) {
                            foregroundColor(!contains(KnifeText.FORMAT_FORGROUND), selectedColor);
                        } else if (view.getId() == R.id.makeBackground) {
                            backgroundColor(!contains(KnifeText.FORMAT_BACKGROUND), selectedColor);
                        }
                    }
                })
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (toolPanel == null) {
            toolPanel = new KnifeToolPanel(context);
            toolPanel.setOnClickListener(toolPanelClickListener);
            //toolPanel.setButtonSize();
        }
    }

    public View getToolPanel() {
        return toolPanel;
    }
    ...
    protected void onSelectionChanged(int selStart, int selEnd) {
        ...
        if (toolPanel != null) {
            if (selStart == selEnd) {
                toolPanel.disableButtons();
            } else {
                toolPanel.enableButtons();
            }
        }
    }
    ...
    // ForegroundColorSpan ===============================================================================
    public void foregroundColor(boolean valid, int color) {
        if (valid) {
            foregroundColorValid(getSelectionStart(), getSelectionEnd(), color);
        } else {
            foregroundColorInvalid(getSelectionStart(), getSelectionEnd(), color);
        }
    }

    protected void foregroundColorValid(int start, int end, int color) {
        if (start >= end) {
            return;
        }
    ...
    protected void foregroundColorInvalid(int start, int end, int color) {
        if (start >= end) {
            return;
        }
        ...
        for (KnifePart part : list) {
            if (part.isValid()) {
                if (part.getStart() < start) {
                    foregroundColorValid(part.getStart(), start, color);
                }

                if (part.getEnd() > end) {
                    foregroundColorValid(end, part.getEnd(), color);
                }
            }
        }
    }

    protected boolean containForegroundColor(int start, int end) {
        if (start > end) {
            return false;
        }
    ...
    // BackgroundColorSpan ===============================================================================
    public void backgroundColor(boolean valid, int color) {
        if (valid) {
            backgroundColorValid(getSelectionStart(), getSelectionEnd(), color);
        } else {
            backgroundColorInvalid(getSelectionStart(), getSelectionEnd(), color);
        }
    }
    ...
    protected void backgroundColorInvalid(int start, int end, int color) {
        if (start >= end) {
            return;
        }
        ...
        for (KnifePart part : list) {
            if (part.isValid()) {
                if (part.getStart() < start) {
                    backgroundColorValid(part.getStart(), start, color);
                }

                if (part.getEnd() > end) {
                    backgroundColorValid(end, part.getEnd(), color);
                }
            }
        }
    }

    protected boolean containBackgroundColor(int start, int end) {
        if (start > end) {
            return false;
        }
    ...
    // Text Size Up ===========================================================================
    public void textsizeup(boolean valid) {
        if (valid) {
            textsizeupValid(getSelectionStart(), getSelectionEnd());
        } else {
            textsizeupInvalid(getSelectionStart(), getSelectionEnd());
        }
    }

    protected void textsizeupValid(int start, int end) {
        if (start >= end) {
            return;
        }
        int size = getTextSize(start, end, true);
        getEditableText().setSpan(new KnifeAbsoluteSizeSpan(size), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }

    protected int getTextSize(int start, int end, boolean isBigger) {
        KnifeAbsoluteSizeSpan[] spans = getEditableText().getSpans(start, end, KnifeAbsoluteSizeSpan.class);
        float textSize, size;
        if (spans.length > 0) {
            size = spans[spans.length - 1].getSize();
        } else {
            //size = (paintTextSize == 0) ? mEditText.getTextSize():paintTextSize; //mEditText.getTextSize();
            size = getTextSize();
        }
        if (isBigger) {
            textSize = (float) (size * 1.1);
        } else {
            textSize = (float) (size * 0.9);
        }
        return (int) textSize;
    }

    protected void textsizeupInvalid(int start, int end) {
        if (start >= end) {
            return;
        }
        ...
        for (KnifeAbsoluteSizeSpan span : spans) {
            list.add(new KnifePart(getEditableText().getSpanStart(span), getEditableText().getSpanEnd(span)));
            getEditableText().removeSpan(span);
        }
        ...
        for (KnifePart part : list) {
            if (part.isValid()) {
                if (part.getStart() < start) {
                    textsizeupValid(part.getStart(), start);
                }

                if (part.getEnd() > end) {
                    textsizeupValid(end, part.getEnd());
                }
            }
        }
    }
    ...
    // Text Size Down ===========================================================================
    public void textsizedown(boolean valid) {
        if (valid) {
            textsizedownValid(getSelectionStart(), getSelectionEnd());
        } else {
            textsizedownInvalid(getSelectionStart(), getSelectionEnd());
        }
    }

    protected void textsizedownValid(int start, int end) {
        if (start >= end) {
            return;
        }
        int size = getTextSize(start, end, false);

        getEditableText().setSpan(new KnifeAbsoluteSizeSpan(size), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }

    protected void textsizedownInvalid(int start, int end) {
        if (start >= end) {
            return;
        }
        ...
        for (KnifePart part : list) {
            if (part.isValid()) {
                if (part.getStart() < start) {
                    textsizedownValid(part.getStart(), start);
                }

                if (part.getEnd() > end) {
                    textsizedownValid(end, part.getEnd());
                }
            }
        }
    }
    ...
    public boolean contains(int format) {
        switch (format) {
            ...
            case FORMAT_FORGROUND:
                return containForegroundColor(getSelectionStart(), getSelectionEnd());
            case FORMAT_BACKGROUND:
                return containBackgroundColor(getSelectionStart(), getSelectionEnd());
            case FORMAT_TEXTUP:
            case FORMAT_TEXTDOWN:
                return false;
            default:
                return false;
        }
    ...
    public JSONObject exportData() {
        ...
        try {
            ...
            JSONObject json = KnifeData.exportData(getText());
            //Log.d(TAG, "> KNIFE_SPANNABLE \n" + JsonFormatter.format(json));
            data.put(KNIFE_SPANNABLE, json);
    ...
    public boolean importData(JSONObject data) {
        ...
        try {
            ...
            JSONObject json = data.optJSONObject(KNIFE_SPANNABLE);
            if (json != null) {
                SpannableString spannableString = KnifeData.importData(json);
