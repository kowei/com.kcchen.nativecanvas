...
public class Filter {
	public ArrayList<FilterOp> op = new ArrayList<FilterOp>();
    public String id;
    ...
    public void optimize() {
        ...
        // as ins SVG specs.It just specifies canvas' options like color filters, without real image composing.
         ...
         ArrayList<FilterOp> newOp = new ArrayList<FilterOp>();

        HashMap<String, FilterOp> results = new HashMap<String, FilterOp>();
        for(FilterOp fa : op) {
        ...
        for(FilterOp f : op) {
            Log.d("SVGKit", "   " + f.filterOp + ", {" + f.in + "," + f.in2 + "} => " + f.result);
        }
        //Log.i("SVGKit", "}Fop");
    }
}
