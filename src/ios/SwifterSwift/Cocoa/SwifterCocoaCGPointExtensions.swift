...
public extension CGPoint {
    ...
    /// SwifterSwift: CGPoint round String
    ///
    ///     let point = CGPoint(x: 10.123456, y: 10.654321)
    ///     let pointString = point.round(at: 2)
    ///     //pointString = (10.12, 10.65)
    ///
    /// - Parameter at: CGPoint to get round at.
    /// - Returns: CGPoint rounded String.
    public func round(at position: Int) -> String {
        let x = String(format:"%.\(position)f", self.x)
        let y = String(format:"%.\(position)f", self.y)
        return "(\(x), \(y))"
    }
