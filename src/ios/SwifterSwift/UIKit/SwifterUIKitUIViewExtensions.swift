...
public extension UIView {
    ...
    func animateTo(scale: CGFloat, origin:CGPoint, withDuration duration: TimeInterval, completion: ((Bool) -> Void)? = nil) {
        guard let _ = superview else {
            return
        }
        
        let x = origin.x + (self.frame.width * scale) * self.layer.anchorPoint.x
        let y = origin.y + (self.frame.height * scale) * self.layer.anchorPoint.y
